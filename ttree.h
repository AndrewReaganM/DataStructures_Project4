...
void ttree::search(string key)
{
    ...
    {
        int index = toInt(key.at(_currentDepth - 1));
        if(!_tnodes[index].nextIsNull()) _tnodes[index].getNext()->search(key);
        if(!_tnodes[index].subSearch(key)) cout << "not found" << endl;
    }
    else if(_currentDepth == key.length()) //If first adequate level, just return items at index.
    {
        ...
        if(_tnodes[index].wordsIsNull() && _tnodes[index].nextIsNull())
        {
            ...
            if ((!_tnodes[index].wordsIsNull())) _tnodes[index].subSearch(key);
            if (!_tnodes[index].nextIsNull()) _tnodes[index].getNext()->search(key);
        }
    }
    else //On further levels, output all tnodes.
    {
        for(int i=0; i < 26; i++)
        {
            if(_tnodes[i].wordsIsNull() && _tnodes[i].nextIsNull())
            {
                ...
                if ((!_tnodes[i].wordsIsNull())) _tnodes[i].display();
                if (!_tnodes[i].nextIsNull()) _tnodes[i].getNext()->search(key);
            }
        }
